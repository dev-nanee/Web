$(function() {
  ...
  $('.gnb .depth01 > dd > a[class!="nobg"],.gnb .depth01 .collec .depth02 > dd > a').
      click(function() {
        if ($(this).next().is(':hidden')) {
          $(this).
              parent().
              siblings('dd[class!="util"]').
              children('dl').
              slideUp();
  ...
  var $contents = $('.sub_top_title_tt, .sub_top_title_tt2'),
      $menu = $('.js-singular-nav > li > a');
  $(window).scroll(function() {
    var scltop = $(window).scrollTop();
    var mNum = 0;
    $.each($contents, function(idx, item) {
      mNum = parseInt($(this).attr('id').substr(-1)) - 1;
      var $target = $contents.eq(idx),
          i = $target.index(),
          targetTop = $target.offset().top - 200;
      if (targetTop <= scltop) {
        $menu.removeClass('b_txt');
        $menu.eq(mNum).addClass('b_txt');
      }
